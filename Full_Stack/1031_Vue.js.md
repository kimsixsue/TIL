...
- HTML 만을 분석하는 것보다 몇 배의 리소스가 필요한 작업이기에 여전히 CSR 의 검색 엔진 최적화 문제가 모두 해결된 것은 아님
...
- **렌더링 된 DOM**을 기본 `Vue` instance의 `data`에 **선언적으로 바인딩** 할 수 있는 **HTML 기반 template syntax를 사용**
...
- HTML의 기본 속성이 아닌 `Vue`가 제공하는 특수 속성의 값으로 `data`를 작성
...
- Vue 의 스타일 가이드 규칙은 우선순위를 기준으로 4가지 범주를 ㅓㄹ정
- 우선순위
  - A: Essential 필수
...
**`v-for` 를 쓴 엘리먼트에 절대 `v-if` 를 사용하지 말기**
...
const app = new Vue({
  ...
  computed: {
    popular() {
      return this.movies.filter((movie) => movie.vote_average >= 8)
    },
  },
